...
| Marketplace listing entitlement | ADR-042 accepted, unimplemented; INV-024 documented (`invariants.md:336-343`) | Not implemented | Implement verbatim (§3.4) |
...
| Marketplace listing | none (F7 visibility gate only) | No — ADR-042 not built |
...
| 7 | **ADR-042 implementation** (§3.4) | Two entity fields, two CLI migrations, grace transitions in the renewal service, shared eligibility policy, indexer gate, Admin schema fields, INV-024 checker | `npm run verify:invariants` includes the new INV-024 sub-check; indexer e2e for eligible/ineligible/in-grace/ineligible-after-grace |
