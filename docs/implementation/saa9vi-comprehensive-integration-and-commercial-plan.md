...
- Scope note for slice 9: the Shop subscription surface is **read-only** — no shop-side purchase/upgrade mutation exists, so purchase wiring is out of slice 9. Upgrades ride ADR-044's Admin mutations, whose *dashboard affordance* is the remaining slice-9 scope.
...
| 5 | **Grant-selection correctness** (§3.3 prerequisites) | 🔶 **PARTIAL (programme) — BUG-036 = ✅ DONE (`d711940`); the slice itself is not closed**, runtime-reproduced against the pre-fix worker: selection restricted to `order`/`subscription` via a positive `IN` list, `isUnbounded` honoured through the shared `grant-selection.policy.ts`, "allowance exhausted" now distinguishable from "no allowance at all", dead `findEarliestValidGrant()` deleted. **Remaining:** plan-derived `concurrentMeetingLimit` sync | BUG-036: `grant-selection.policy.spec.ts` 8/8 + 3 new real-Postgres cases in `bbb-meeting-concurrency.e2e-spec.ts` (each fails pre-fix, 4/4 post-fix); `bbb-usage-ledger.e2e-spec.ts` 5/5. **Adjacent finding while re-running the BBB suites as regression cover: BUG-037** — `bbb-channel-isolation.e2e-spec.ts` (the INV-001 runtime evidence) could not pass at HEAD (7 failed/6 skipped: missing `CmsPlugin`/`ReviewsPlugin`/`SubscriptionPlugin` in the harness, missing BUG-033's `requireVerification: false`, and an org-creation phase racing an un-awaited `asSuperAdmin()` + `setChannelToken('')` while `BbbTenantProvisioningListener` had already provisioned the org). Fixed harness-only; suite now **13/13**. Still open: one runtime session attempt at the limit |
| 6 | **Daily allowance layer** (§3.3) | ⏳ **OPEN — blocked by 5** (the matrix itself is frozen in slice 1). Scheduled daily-grant job with the `(org, startOfDay, subscription)` idempotency key; allowance read model. `consumeGrantHours` and `BbbUsageLedger` untouched | Runtime: 2 sessions in one day, second refused with the entitlement error; ledger rows append-only |
...
| 9 | **Frontend** (§3.8) | ⏳ **OPEN** (separate repo; needs slice 8's schema first) — Theme consumption + plan/usage dashboard | Runtime A/B isolation, marketplace/admin default theme, unknown hostname fails closed |
| 10 | **R4 runtime evidence** (§3.10) | ⏳ **OPEN** (verification only — R4 is implemented) — End-to-end proof with the dummy handler + explicit settle | Evidence recorded in `production-readiness.md`, not here |
