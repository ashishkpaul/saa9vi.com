...
| 8 | **Shop read contract** (§3.5) | ⏳ **OPEN — nothing blocks it** (parallelisable with slice 5) — `mySubscription`, `myLiveUsage`, `availableSubscriptionPlans` (Public/Authenticated as appropriate); codegen regenerated | Tenant A/B isolation test; no `channelId` argument anywhere in the new surface |
