...
**Slice-5 implementation baseline (newest *source* commit):** `0a6b433` (`0a6b433ce2d97a365d817da62963516cfb7587b2`, 2026-09-25) — Slice 5: plan-derived `concurrentMeetingLimit` + BUG-039 fix. Every later commit is **docs-only** and changes no file under `src/` (`6e4467a`, 2026-09-25; the second reconciliation pass, 2026-09-25), so `0a6b433` remains the behavioural baseline for every claim in this document and a newer docs HEAD does not invalidate any of them. This line is deliberately **not** maintained as a "current HEAD" pointer — that is what `git rev-parse HEAD` is for.
