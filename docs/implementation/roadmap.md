...
**Position (plan §4 order):** slices 1, 2, 3, 4, 7, 8, 9 and **10** are done; slice 5 is **partial** (BUG-036 grant-selection correctness ✅ `d711940`; plan-derived `concurrentMeetingLimit` sync outstanding). **Slice 10 — R4 runtime evidence ✅ DONE 2026-09-25** (commit `4870251`): one infrastructure-gated spec, `R4_E2E=true` → **10/10** on real Postgres, and producing it found and fixed **BUG-038** — `bbbFulfillmentHandler` resolved lines from the never-loaded `order.lines`, so the purchase path had never once written an `order`-source capacity grant. R4 evidence lives in §10 of `production-readiness.md`; refund/reversal semantics and reconciliation remain open there. **Next: slice 6 — daily live allowance**, still open behind the slice-5 concurrency item; slice 11 (R3 one-time payment handler) is an independent gate.
