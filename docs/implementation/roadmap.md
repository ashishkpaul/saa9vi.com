...
- [⚠️] **3D.1b Step 9 (failure/recovery + convergence E2E)** — implemented but **NOT yet verified**: `convergence.e2e-spec.ts` (real PostgreSQL + Redis/BullMQ + Elasticsearch via cloudflared tunnels) currently has **1/4 passing** (2026-09-06). Failure signatures: (a) `AssertionError: expected undefined to be defined` in the refresh→reindex→converge, missing-doc self-heal, and supersession cases; (b) `GraphQLError: error.no-configurable-operation-def-with-code-found` during fixture setup. Intended coverage (to be restored green before 3D.1b completion): full convergence `stale=0`; missing-doc self-heal via re-run of the same target version; ineligible ES doc does not inflate `total`; supersession aborts a stale-version reindex and the new version converges. Scale note: `globalReindex` remains sequential per-session; bounded concurrency/bulk indexing deferred to Phase 4.
