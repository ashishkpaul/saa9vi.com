...
5. Grant-selection correctness (`internal_overhead` vs exhaustion; plan-derived `concurrentMeetingLimit`). — 🔶 **PARTIAL** (BUG-036 `d711940`, BUG-035 `ab274bf`, BUG-037 `30d245a` closed — plan §4 row 5; **the plan-derived `concurrentMeetingLimit` sync remains**, carried as a slice-6 prerequisite).
