...
| BUG-026 | Medium | `tenant-admin.resolver.ts` | Vendure's built-in singular `role(id)` and `administrator(id)` queries are also implicitly channel-scoped (same gap as BUG-025/INV-016). A SuperAdmin on the Default channel gets "not found" when clicking into a tenant role or administrator detail page. Fixed by overriding both singular queries with the same SuperAdmin-bypass pattern. | ✅ Fixed |
