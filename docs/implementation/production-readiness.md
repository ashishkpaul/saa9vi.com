...
| RFC-001 §4 specified a separate `RecurringCapacityGrant` entity | `adr/rfc-001-continuous-commerce-loop.md` | Reconciliation note: shipped design uses the `sourceType: 'subscription'` discriminator; `GrantReaderService` closed the Q-009 seam; the harness name mismatch — which produced one permanent `verify:invariants` warning — was reconciled 2026-09-24 at `f0e5ece` (RFC-001 v4 amendment + canonical `SubscriptionCapacityGrant` action), clearing the warning |
...
**Free Basic activation — runtime evidence (2026-09-23; re-run 2026-09-25).** `scripts/verify/free-basic-activation.sh` was executed against a live server (Vendure 3.6.5 on :3000) → **19 passed / 0 failed / 3 skipped, exit 0** on 2026-09-23, and **24 passed / 0 failed / 3 skipped, exit 0** on the 2026-09-25 re-run after Slice 5 added step 6b (`source=plan`, `maxConcurrentMeetings = 1`, `org.concurrentMeetingLimit = 1` — the plan-derived concurrency ceiling converging onto the organization). A real Shop-API `registerNewTenant` produced **exactly one** `OrganizationSubscription` for the new channel — status `active`, plan `free-basic`, `providerPlanId` NULL, `currentPeriodStart`/`currentPeriodEnd` **NULL**, `cancelAtPeriodEnd` false — with **zero** `SubscriptionProviderBinding` rows and **zero** billing attempts (no provider interaction at all), and `subscribeToPlan` was correctly refused with the ADR-044 guard message naming `changeOrganizationSubscriptionPlan`. The `SKIP`s are structural: duplicate `TenantRegisteredEvent` delivery is not replayable over GraphQL, and the renewal sweep is scheduler-driven; both carry manual procedures in the script. The Free tier's `BbbPlatformCapacityPolicy` row is created idempotently by scenario 2 (originally 5/5/5; scenario 2 now also sets **and asserts** `maxConcurrentMeetings = 1`, the frozen Free Basic ceiling).

**ADR-042 marketplace entitlement — implemented (2026-09-23, plan slice 7 / §3.4).** Both columns exist (`subscription_plan.marketplaceListingEnabled` BOOLEAN NOT NULL DEFAULT **false**; `organization_subscription.marketplaceGraceUntil` TIMESTAMP NULL) — verified in the database after running **two Vendure-CLI-generated** migrations (`npx vendure migrate -g`, never hand-written). Eligibility is evaluated by exactly one policy, the platform-level `CommercialEntitlementService` (`src/platform/commercial/`), whose window is supplied per entitlement — so the ADR-043 theming window (`{trialing, active, past_due}`) and the ADR-042 window (`{active} ∪ {past_due ∧ now() < marketplaceGraceUntil}`) cannot drift while sharing one evaluator; `TenantCommercialEligibilityService` now delegates to it (behaviour-preserving: 73/73 tenant e2e against real Postgres). `MarketplaceIndexerService.indexSession()` applies the gate alongside F7 (ineligible ⇒ document removed; `false` is not an error), and grace deadlines are written **only** by `SubscriptionRenewalService` (set on entry to `past_due` — webhook *and* local failed-charge transitions — cleared on recovery/cancellation, `MARKETPLACE_GRACE_PERIOD_DAYS` default 7). The INV-024 structural-checker gap recorded in the 2026-09-22 drift sweep is now closed: `AdrChecker.marketplaceEntitlementInvariants()` runs under `npm run verify:invariants` (green). Evidence (commit `dad099c`): **14/14 `marketplace.e2e-spec.ts` executed against real Postgres + real Elasticsearch** — the ADR-042 matrix runs green (a plan with `marketplaceListingEnabled=false` delists an active channel; `past_due` inside `marketplaceGraceUntil` stays listed; `past_due` after the deadline delists; NULL grace delists; no subscription row delists as `false`, not an error; PROHIBITED signals cannot change the decision; eligibility restored), alongside the pre-existing projection / removal / rating / ledger tests; **4/4 `convergence.e2e-spec.ts`** (equivalent seeding, same real infra); 25/25 infra-free gate spec (real policy + real indexer, incl. prohibited-signal probes); 73/73 tenant e2e (real Postgres, delegated whitelabel-window regression); `npm run verify:invariants` green (INV-024 sub-check). Two e2e-only defects were found and fixed en route: the fixtures passed the GraphQL-encoded id form (`T_1`) where the queue contract delivers the raw PK, and the new `DRAFT` session default meant fixtures had to pin `{visibility: PUBLIC, status: SCHEDULED}` or the F7 gate silently pruned every document. Known residual (documented in the ADR): instructor documents are outside the ADR's §4 scope.
