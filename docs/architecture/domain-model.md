...
- ✅ **Fixed (2026-09-23, `d711940`):** the provisioning-time capacity check now honours `isUnbounded` and never selects `internal_overhead` capacity for a tenant session. `GrantReaderService.getRemainingMinutes()` shares the same policy helper (`services/grant-selection.policy.ts`). An exhausted commercial allowance now refuses with its own accurate message instead of falling through to the overhead grant. Tests: `grant-selection.policy.spec.ts` 8/8 (infra-free); `bbb-meeting-concurrency.e2e-spec.ts` 3 new real-Postgres cases, all failing pre-fix and 4/4 post-fix. Tracked as **BUG-036** (now closed) from **F-4** in `docs/implementation/saa9vi-comprehensive-integration-and-commercial-plan.md` §0.19.
- Historical note (pre-fix, 2026-09-22): the provisioning-time capacity check did **not** honour `isUnbounded` — `doProvisionMeeting()` selected on `exhausted = false` + validity window and then rejected when `grantedMinutes - consumedMinutes <= 0`, while only `GrantReaderService.getRemainingMinutes()` special-cased unbounded grants. An org whose only valid grant was the unbounded overhead grant therefore failed provisioning with "No minutes remaining on plan", and an exhausted commercial grant fell through to the overhead grant.
