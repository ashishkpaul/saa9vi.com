...
Then, in **plan §4 order** (slice numbers are the plan's canonical ones; the FREE-1 list in `integration-gaps-worklist.md` uses the same order): ✅ 1 commercial-matrix freeze → ✅ 2 doc-drift sweep → ✅ 3 ADR-044 + plan-change/cancel (`66e6cd4`; acceptance harness repaired `5d3d2d9`) → ✅ 4 provider-free Free Basic activation at registration (`d45b0a5`; runtime-verified 19 passed / 0 failed / 3 skipped, re-run 2026-09-25 → 24 passed / 0 failed / 3 skipped) → ✅ 5 grant-selection correctness — DONE 2026-09-25 (BUG-036 fixed `d711940` (incl. dead-seam `findEarliestValidGrant` removal); BUG-037 isolation harness repaired `30d245a`; plan-derived `concurrentMeetingLimit` sync delivered, which surfaced and fixed BUG-039) → ✅ 6 daily live allowance (ADR-045 / INV-026, `07aa9d6`) → ✅ 7 ADR-042 marketplace entitlement (`dad099c`; indexer e2e matrix **14/14 against real Postgres + Elasticsearch**, convergence **4/4**, tenant regression 73/73, INV-024 green) → ✅ 8 Shop read contract (`ec866fe`; 12/12 `subscription-shop.e2e-spec.ts`, locked permission model) → ✅ 9 `edu-frontend` theme + dashboard (theme ADR-043 L1 `7561643`; dashboard + admin sign-in bridge `b262ba3`; template-contract doc `2bba7e2` — all pushed; Admin-authenticated business session → Shop reads only; upgrade/cancel mutations stay deferred with UI-1/ADR-044) → ✅ 10 R4 runtime evidence (`4870251`; `R4_E2E=true` 10/10) → 11 R3 payment handler. **R3 (`dummyPaymentHandler` is still the only registered handler) is a separate launch gate**, unaffected by this programme. **Product decisions: the free-tier matrix was frozen in slice 1 (2026-09-22) — no open product decisions remain inside §4**; UI-1 (upgrade/cancel affordance) and the R3 handler choice sit outside this programme. Detail, gates and evidence in §Remaining backlog below.
...
| 9 | **Frontend: the `edu-frontend` theme + plan/usage dashboard** (§3.8) | ✅ **DONE 2026-09-24** (`edu-frontend` `7561643` + `b262ba3` + `2bba7e2`, pushed) — ADR-043 L1 theme consumption (`myTenantTheme` → CSS-var payload, edge injection keyed on `x-saa9vi-channel-token`, ineligible ⇒ `null` ⇒ platform default, unknown host fails closed) + `/account/billing` (status/period/cancel states, usage panel rendering standalone when `mySubscription` is `null`, plans grid with current-plan ring) reading only slice-8 Shop queries; sign-in bridge authenticates business accounts on the **Admin API** (`VENDURE_ADMIN_API_URL`, documented in `.env.example`) and presents the session token to Shop reads; **no mutations** (stay deferred with UI-1/ADR-044); zero commercial/business-rule logic in the client | Runtime evidence (dev :3001): final-tree gates markers/JSON/codegen:check/tsc/eslint/**build all exit 0**; browser E2E — registered tenant admin verified via `verifyTenantAdmin` → Admin-API session → storefront bridge sign-in (cookie validated by admin `me`, redirect lands on `/en`) → billing renders Free Basic active + NULL period + "Your current plan" + usage 0/0/0 on channel 25 (screenshot `/tmp/bill_owner2.png`); tenant isolation — apex usage 120/11/109 standalone with `mySubscription: null` (grant-only allowance), g2-alpha paid active with period + marketplace flag, freebasic marketplace not-listed, catalogue plans real (₹499.00 / ₹0.00); bogus token → fail-closed "unavailable" panel; unmapped host → 403 / `channelToken: null` (B-6, unchanged); theme positive render untestable in dev (no channel has an active theme row — injection returns `null`, tenant gate opens; backend fixture gap, not a frontend gap) |
| 10 | **R4 runtime evidence** (§3.10) | ✅ **DONE 2026-09-25** (`4870251`; `R4_E2E=true` → **10/10** on real Postgres; producing it found and fixed **BUG-038**) | Closed — evidence in §10 of `production-readiness.md`; refund/reversal semantics remain open there |
| 11 | **R3 payment handler** (§3.9) | ✅ Complete — `PaymentsPlugin` registered, Razorpay one-time handler stack, Shop mutation, webhook endpoint, Option A auto-fulfillment (`R4_E2E=true` 10/10) | Independent launch gate |

### Slice 5 — grant-selection correctness + plan-derived concurrency — ✅ DONE 2026-09-25 (BUG-036/035/037/039 closed — plan §4 row 5)

**Closed:** BUG-036 fixed in `d711940` (never serve tenant sessions from `internal_overhead` capacity; `isUnbounded` honoured at the provisioning gate), the dead seam (`findEarliestValidGrant`, ignored `_sourceTypes` + validity window) removed in `d711940` (same pass), and the downstream channel-isolation harness repaired in `30d245a` (BUG-037 — **harness-only**: the production `TenantRegisteredEvent → BbbTenantProvisioningListener → tenant-scoped ctx → assignToCurrentChannel()` path was never changed, and no product code should be "fixed" on the basis of the former failing test). Runtime-reproduced 2026-09-23/24; acceptance evidence: meeting-concurrency **4/4** (`MEETING_CONCURRENCY_E2E=true`), usage-ledger **5/5** (`BBB_USAGE_LEDGER_E2E=true`), isolation **13/13**. `known-bugs.md` Active = None. The analysis below is retained as the pre-fix record (verified at `6c86a0a`).

**Plan-derived `concurrentMeetingLimit` — the slice's remaining half, delivered 2026-09-25.** Free Basic's frozen ceiling of **1** concurrent room is now *derived*, not form-supplied: `BbbPlatformCapacityPolicy.maxConcurrentMeetings` (entity + CLI migration `1790319702564`) feeds `EffectiveCapacityPolicy`, and a single write-through `syncConcurrentMeetingLimit()` pushes it onto `BbbOrganization.concurrentMeetingLimit` — guarded by `isPlanDerived()` so a Tier 3/4 resolution can never overwrite an Admin-set value, while a Tier 1 channel-override still applies because it *is* plan-derived. It is invoked from org `create()`/`update()`, from `SubscriptionPlanChangedEvent` (published by `SubscriptionService.announcePlanChange()` and `FreePlanProvisioningService.announceActivation()`, both warn-and-continue) and from a startup reconciliation bootstrap — **convergence, not listener ordering**, because `BigBlueButtonPlugin` registers before `SubscriptionPlugin`, so either `TenantRegisteredEvent` handler may legitimately resolve first. ADR-031 was amended (Proposed → Active, tier table "Max Concurrent Rooms" column, Decisions 1–5) and INV-015 extended.

**Producing it surfaced BUG-039** (fixed same day): Tier 2's raw `organization_subscription` lookup resolved through `search_path` while entity queries resolve through `schema`, so plan-derived concurrency silently fell through to `fallback` whenever a schema was configured. Evidence: `plan-derived-concurrency.e2e-spec.ts` (`PLAN_CAPACITY_E2E=true`) pre-fix **2/5** → post-fix **5/5**; `scripts/verify/free-basic-activation.sh` extended (step 2 seeds *and asserts* `maxConcurrentMeetings: 1`, new step 6b polls the org → 1) and re-run on a live server **24 passed / 0 failed / 3 skipped** (was 19/0/3), with 6b reporting `source=plan`. Full battery on the frozen tree: build / `verify:invariants` (100/100, 0 warnings) / lint / `tsc` all 0, tenant **73/73**, isolation **13/13**, meeting-concurrency **4/4**, usage-ledger **5/5**, subscription-shop **12/12**, R4 **10/10**. **Still open inside the slice's original scope:** one *runtime* session attempt asserting `assertCanCreateMeeting` throws at the limit (provisioning-side concurrency is covered).
...
Acceptance: honour `isUnbounded` at the provisioning gate (Infinity semantics, matching `getRemainingMinutes()`); make `internal_overhead` explicitly non-selectable for tenant sessions; an exhausted commercial allowance must refuse with a domain-accurate outcome, not a generic error; unit + BBB e2e green (`bbb-meeting-concurrency.e2e-spec.ts` covers provisioning); close BUG-036 in `known-bugs.md` with the commit hash. — **all criteria met: `d711940`, `known-bugs.md` updated, Active = None.**
