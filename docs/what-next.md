...
| **R2-F** Durable Processing | ⚠️ Proven (pre-refactor) | BullMQ inbox worker, single processing path — processor write path materially changed in the provider-neutral refactor (service-delegated attempts, single atomic terminal CAS write carrying provider IDs, replay-safe finalize, FIFO reconciliation). Post-refactor runtime recapture required |
...
| **R2-F** Processing Idempotency | ⚠️ Proven (pre-refactor) | Double-send → single billing attempt — **recapture required**: a terminal attempt is resolved by `providerEventId` **before** any event dispatch, so a same-event replay of a terminal `succeeded` attempt re-runs the replay-idempotent `finalizeAfterPayment()` (a crash between the terminal write and finalize is recovered); a terminal `failed` attempt is a no-op. The `providerPaymentId` unique-violation race converges through the same reconciliation helper. Success metadata is written in one atomic CAS transition |
...
| **R2-G** Failure Semantics | ⚠️ Proven (pre-refactor) | pending → retry → failed (terminal), `failedAt` — captured pre-refactor. Post-refactor the processor additionally bridges `subscription.pending`/`subscription.halted` → Saa9vi `past_due` (dunning entry point, RFC-001 §4.2) and `subscription.cancelled` → `cancelled`; the bridge CAS increments `version` and only transitions from `active`/`trialing`. This FSM bridge is **CODE VERIFIED, not RUNTIME VERIFIED**, and an **OUT-OF-ORDER webhook test is required** (Razorpay does not guarantee delivery order) — capture during R2-E/R2-G re-verification |
