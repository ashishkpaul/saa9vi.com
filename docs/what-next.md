...
| **R2-F** Durable Processing | ⚠️ Proven (pre-refactor) | BullMQ inbox worker, single processing path — processor write path materially changed in `72961d6`→`c9f4a1c` (service-delegated attempts, atomic CAS + provider IDs, replay-safe finalize, FIFO reconciliation). Post-refactor runtime recapture required |
...
| **R2-F** Processing Idempotency | ⚠️ Proven (pre-refactor) | Double-send → single billing attempt — **recapture required**: `isEventProcessed()` requires a TERMINAL attempt match, the success path is a single atomic CAS transition, and terminal-succeeded replays re-run the (now replay-idempotent) finalize — post-`c9f4a1c` |
