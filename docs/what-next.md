...
| **R2 — Razorpay Contract Verification** | ⚠️ Code Hardened / Runtime Re-Verify | ADR-041 G2–G7 complete (2026-09-20). BUG B (two-month period drift) found by R2-E probe and fixed. Two migrations pending apply. R2-E/F/G runtime re-verification required against ADR-041 code with a fresh Test-mode subscription. See `production-readiness.md` R2-E/F/G and `known-bugs.md` BUG B. |
...
| **R2-E** Authorization | ⚠️ CODE HARDENED / RE-VERIFY REQUIRED | R2-E probe (2026-09-20) ran against pre-ADR-041 code and found **BUG B** (two-month period drift). ADR-041 G2–G7 code hardening complete 2026-09-20 (`npx tsc --noEmit` ✅, `npm run build` ✅, 6 e2e pass). Fresh Test-mode subscription required. Two migrations pending apply (`1789883158253`, `1789885988242`). See `production-readiness.md` R2-E and `known-bugs.md` BUG B. |
| **R2-F** Durable Processing | ⚠️ CODE HARDENED / RE-VERIFY REQUIRED | Webhook processor hardened by ADR-041 (cycle-monotonic CAS, transactional `updateBinding`, provider-qualified binding lookup, bounded retry max 3, `assertProviderCyclePresent` fail-closed). Post-ADR-041 runtime lifecycle recapture required. |
| **R2-F** Channel Resolution | ✅ Proven (scoped) | Resolved from an **existing** provider binding (INV-001). First-subscription binding **creation** lifecycle **CLOSED 2026-09-16** as **C-1** in `integration-gaps-worklist.md` — all C-1 sub-gates (A/B/C/C-idempotency/D/E) runtime-verified post-fix |
| **R2-F** Inbox Idempotency | ✅ Proven | UNIQUE(provider, providerEventId) — inbox-level, unaffected by the processor refactor |
| **R2-F** Processing Idempotency | ⚠️ CODE HARDENED / RE-VERIFY REQUIRED | ADR-041: replay check uses `currentPeriodStart >= targetStart` (cycle-identity); cycle-monotonic CAS; terminal attempt idempotency unchanged. Recapture required against ADR-041 code. |
| **R2-F** Concurrent Idempotency | ⚠️ CODE HARDENED / RE-VERIFY REQUIRED | DB UNIQUE constraint unchanged; CAS race bounded retry (max 3). Recapture alongside the R2-F runtime lifecycle. |
| **R2-G** Failure Semantics | ⚠️ CODE HARDENED / RE-VERIFY REQUIRED | ADR-041 G6: `requireProviderCycleForFailure()` fail-closed; cycle-identity freshness guard replaces wall-clock guard; uniform NULL period semantics for failed attempts. Out-of-order webhook test required. D-5 (halted recovery) still open. |
