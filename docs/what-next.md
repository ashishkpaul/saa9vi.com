...
|---|---|---|---|
| 5 | **Grant-selection correctness** (§3.3 prerequisites) — BUG-036 | 🔶 **PARTIAL** (plan §4 row 5) — BUG-036 fixed `d711940` + `ab274bf` (BUG-035 dead seam `findEarliestValidGrant` removed); runtime-reproduced 2026-09-23/24; isolation harness repaired `30d245a` (BUG-037); `known-bugs.md` Active = None; **remaining: plan-derived `concurrentMeetingLimit` sync** | `grant-selection.policy.ts` positive `IN` list is the single home (provisioning gate + slice 8's `myLiveUsage`); the concurrency sync is carried as a slice-6 prerequisite |
| 6 | **Daily live allowance** (§3.3) | Open — **unblocked** (5's BUG-036/035 closed; the slice-5 `concurrentMeetingLimit` sync rides inside §3.3's prerequisite list; matrix frozen in slice 1) | Reuse `BbbSubscriptionListener`/`BbbCapacityGrant` — F-6 closed (`f0e5ece`), no second grant writer; slice 8 already reports period usage read-side; the §3.3 provisioning-side daily grant remains to build |
...
### Slice 5 — grant-selection correctness — 🔶 PARTIAL (BUG-036/035/037 closed 2026-09-23/24; plan-derived `concurrentMeetingLimit` sync remains — plan §4 row 5)
