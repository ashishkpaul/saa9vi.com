...
- **Rendering** — every list/ledger screen derives its render state from `resolveListState()` (`src/platform/dashboard/query-state.ts`) and renders through `LedgerStateView` (`src/plugins/subscription/dashboard/shared/query-state-panel.tsx`), which separates `loading` / `error` (with Retry) / `blocked` (the query was deliberately not issued) / `empty` / `ready`. Missing-item normalisation happens only inside that resolver, so a disabled query, a rejected operation or a reshaped response can no longer reach the empty-state branch, and a screen's `N total` footer renders only in the `ready` branch (a `0 total` under a failed query is the same false signal as an empty table). The resolver is framework-free and lives in the server project so the regression spec can import it directly without crossing the Dashboard TS project boundary.
- **Filter vocabulary** — status options come from `src/platform/dashboard/billing-vocabularies.ts` and are re-derived from the code that writes them (`RazorpayWebhookProcessor.updateBinding()` literals, the `BillingAttemptStatus` union, the `ReconciliationIncidentStatus` enum). A selectable value the backend cannot persist is a phantom filter that always answers with an empty table.
- Regression spec — `src/plugins/subscription/__tests__/dashboard-query-state.spec.ts`: resolver outcomes, source-derived vocabularies, and a **shrink-only baseline** of screens that still fall back to `items ?? []` with no error branch at all (currently 9 BBB / reviews / tenant-plugin screens; a new one fails the suite).

**Rejection criterion:** Any Dashboard document referencing a field, argument or type absent from the Admin schema is rejected. Any ledger or list screen that renders a query failure as an empty dataset is rejected. Any filter control offering a status value no code path can persist is rejected.
